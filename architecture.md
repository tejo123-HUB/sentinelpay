...
| AI Assistant serving (production target / optional enhancement) | Anthropic Claude API (`claude-sonnet-5`) via native `fetch`, no SDK dependency — used only when `ANTHROPIC_API_KEY` is configured, always falls back to the deterministic path on any failure |
...
*Last confirmed accurate as of: 18 July 2026, Partial-Feature Completion Pass.*
